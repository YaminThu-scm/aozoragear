...
theme.productCollSwatch = function() {
	function createSrcsetString(srcSetWidths, url, originalWidth, originalHeight) {
 	...
 	$('.product-card__swatch__item').on('click', function() {
		...
		var currentProductImage = currentProduct.find('.product-card__img');
...
theme.productVariantAvailability = function(product, form, thisVariant, cssClass) {
	...
	//creating a 2 dimentional array that available options will be added to
	...
	// Visually display unavailable swatch buttons
	$(form).find('.product-form__swatch__item').addClass('product-form__swatch__item--disabled');
...
theme.productSelect = function(productId, sectionId, cssClass, historyState) {
	...
	var selectCallback = function(variant, selector) {
		...
		var productForm = thisSection.querySelector('#js-product-form--' + productId);
	...
	if (request.status >= 200 && request.status < 300) {
	  ...
	  if ($("body").data("anim-load")) {
	  	if (typeof sr !== 'undefined') {
	  		sr.reveal('.section--related-products .product-card-top', { interval: theme.intervalValue });
